...
def is_factor_of n m = eq (rem m n) zero

def count_factors_iter n k acc =
    match k
        with zero => acc
        with succ k' => (
...
def count_factors n = count_factors_iter n n zero

def is_prime n = eq (count_factors n) two
